...
const playerSocketCounts = new Map();
io.on('connection', (socket) => {
    console.log('A user connected:', socket.id);
    const broadcastGameState = (sessionId) => {
        ...
        for (const socketId of sockets) {
            ...
            if (isDirector) {
                ...
                // Sanitize for players: if not active, show the previous round from history
                if (!session.isRoundActive && session.rounds.length > 0) {
                    const index = session.rounds.length - 1;
                    const lastRound = session.rounds[index];
                    ...
                    s.emit('gameStateUpdate', sanitizedState);
                }
                else if (!session.isRoundActive && session.round_number === 1 && !session.currentRound) {
                    ...
                    // Active round or no history to fallback to
                    s.emit('gameStateUpdate', { ...session, aiEnabled: llmService.isAvailable });
                }
            }
        }
        ...
        if (success) {
            ...
            if (session) {
                ...
                if (session.currentRound) {
                    io.to(sessionId).emit('newRound', session.currentRound);
                }
            }
        }
    });
        ...
        for (const room of socket.rooms) {
            if (room === sessionId) {
                ...
                if (session) {
                    broadcastGameState(room);
                    if (session.currentRound) {
                        io.to(room).emit('newRound', session.currentRound);
                    }
                }
            }
        }
    ...
    const handleGenerateNextRound = async (sessionId, initiatorSocket) => {
        const session = gameManager.getSession(sessionId);
        if (!session || session.status === 'WAITING_AI')
            return;
        ...
        try {
            ...
            const llmResponse = await llmService.generateNextRound(session.rounds, session.players, recentMessages, session.currentRound, session.directives);
            console.log('LLM Response generated:', llmResponse);
            let updatedSession = gameManager.applyNextRoundUpdates(sessionId, llmResponse.description, llmResponse.characterUpdates, llmResponse.gameSummary, llmResponse.goals);
            if (updatedSession) {
